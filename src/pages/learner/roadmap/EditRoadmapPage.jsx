...
import { getMyRoadmap, saveRoadmap } from "../../../services/roadmapService";
...
export default function EditRoadmapPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!res) {
      console.log(payload);
      addToast("Không thể lưu lộ trình, vui lòng thử lại", "error");
      return;
    }
    await fetchData();
    addToast("Thành công lưu lộ trình", "success");
  };
    ...
    <div className="p-5 h-[85vh] flex flex-col items-center justify-between font-sans text-slate-800">
      <div className="w-full flex flex-col items-center">
        <h1 className="text-2xl font-extrabold mb-25 tracking-tight text-blue-500">
          {steps.length !== 0 ? (
            "Chỉnh sửa lộ trình học tập của bạn"
          ) : (
            <>
              Bạn chưa có lộ trình học tập để chỉnh sửa?{" "}
              <a
                href="all"
      ...
      {steps.length !== 0 && (
        <div className="w-full flex justify-end-safe mr-10 gap-5">
          <button
            ...
            className="bg-blue-500 hover:bg-blue-600 hover:shadow-lg transition px-5 py-2 text-white font-semibold"
          >
            Lưu chỉnh sửa
          </button>
          <button
            onClick={deleteRoadmap}
            className="bg-[#cf345a] hover:bg-[#c71c46] hover:shadow-lg transition px-5 py-2 text-white font-semibold"
          >
            Xóa lộ trình
          </button>
        </div>
